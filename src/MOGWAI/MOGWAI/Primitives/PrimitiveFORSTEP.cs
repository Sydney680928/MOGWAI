...
{
    internal class PrimitiveFORSTEP : MOGPrimitive
    {
        public PrimitiveFORSTEP(MogwaiEngine engine, string name) : base(engine, name)
        {
        ...
        {
            var obj = new PrimitiveFORSTEP(Engine, Name);
        ...
        public override async Task<EvalResult> EngineEval()
        {
            ...
            if (s[0] == typeof(MOGCode) && s[1] == typeof(MOGName) && s[2] == typeof(MOGNumber) && s[3] == typeof(MOGNumber) && s[4] == typeof(MOGNumber))
            {
                ...
                var step = Engine.StackPopNumber();
                ...
                for (double i = start.Value; direction > 0 ? i <= end.Value : i >= end.Value; i += step.Value)
                {
