...
{
    internal class PrimitiveFileOpenIn : PrimitiveParamsString
    {
        public PrimitiveFileOpenIn(MogwaiEngine engine, string name) : base(engine, name)
        {
        ...
        {
            var obj = new PrimitiveFileOpenIn(Engine, Name);
        ...
        public override Task<EvalResult> PerformOperation(MOGString @string)
        {
            ...
            try
            {
                var reader = File.OpenRead(@string!.Value);             
                var id = reader.GetHashCode().ToString("X");
