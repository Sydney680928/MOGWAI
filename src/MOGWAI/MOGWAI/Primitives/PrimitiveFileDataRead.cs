...
{
    internal class PrimitiveFileDataRead : PrimitiveParamsString
    {
        public PrimitiveFileDataRead(MogwaiEngine engine, string name) : base(engine, name)
        {
        }

        public override MOGObject Clone()
        {
            var obj = new PrimitiveFileDataRead(Engine, Name);
        ...
        public override async Task<EvalResult> PerformOperation(MOGString @string)
        {
            ...
            try
            {
                var b = File.ReadAllBytes(@string.Value);
