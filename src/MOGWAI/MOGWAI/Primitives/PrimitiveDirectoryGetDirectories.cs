...
{
    internal class PrimitiveDirectoryGetDirectories : PrimitiveParamsString
    {
        public PrimitiveDirectoryGetDirectories(MogwaiEngine engine, string name) : base(engine, name)
        {
        }

        public override MOGObject Clone()
        {
            var obj = new PrimitiveDirectoryGetDirectories(Engine, Name);
        ...
        public override Task<EvalResult> PerformOperation(MOGString @string)
        {
            ...
            try
            {
                var items = Directory.GetDirectories(@string.Value);
