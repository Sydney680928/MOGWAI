...
{
    internal class PrimitiveUnpack : PrimitiveParamsData
    {
        ...
        public override Task<EvalResult> PerformOperation(MOGData data)
        {
            ...
            try
            {
                var bytes = Engine.Decompress(data.Items.ToArray());
                var str = Encoding.UTF8.GetString(bytes);
                var parser = new Parser();

                Engine.AllowExtendedNames = true;
                parser.Parse(Engine, str, 0, ExecutionContext);
                Engine.AllowExtendedNames = false;
