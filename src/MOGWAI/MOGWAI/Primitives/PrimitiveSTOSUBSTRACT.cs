...
{
    internal class PrimitiveSTOSUBSTRACT : MOGPrimitive
    {
        public PrimitiveSTOSUBSTRACT(MogwaiEngine engine, string name) : base(engine, name)
        {
        ...
        {
            var obj = new PrimitiveSTOSUBSTRACT(Engine, Name);
        ...
        public override async Task<EvalResult> EngineEval()
        {
            ...
            if (s[0] == typeof(MOGName) && s[1] == typeof(MOGNumber))
            {
                ...
                if (value == null)
                {
                    Engine.VarWrite(name.Value, new MOGNumber(Engine, -number!.Value, 0));
                }
                else
                {
